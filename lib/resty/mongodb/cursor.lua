...
--[[
-- 由于hash table在lua中是无序的，需要手动指定顺序
-- @param order   table {key1 = -1, key2 = -1}
-- @param by_keys table {"key1", "key2"}
--]]
function cursor:sort(order, by_keys)
    assert_state(self)
    
    -- 构建元方法 __pairs
    if by_keys and type(by_keys) == 'table' then
        setmetatable(order, {__pairs = function (t)
            local i = 0
            return function ()
                i = i + 1
                return by_keys[i], t[by_keys[i]]
            end
        end})
    end
...
function cursor:kill_cursor()
    local id = self.id
    if id then
        local _, m = mongo_killcusors_message({ id })
